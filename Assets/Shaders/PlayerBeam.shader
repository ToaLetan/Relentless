Shader "Custom/PlayerBeam"
{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			{
				//The following code applies a flickering effect to the player's beam
				float4 result = tex2D( _MainTex, IN.texcoord) * _Color;

				float fadeValue = _SinTime.w;

				result.a = clamp(fadeValue, 0.2f, 0.8f);
